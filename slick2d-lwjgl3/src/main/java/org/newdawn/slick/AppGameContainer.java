...
public class AppGameContainer extends GameContainer {
    ...
    /**
     * @see org.newdawn.slick.GameContainer#setMouseCursor(org.lwjglx.input.Cursor, int, int)
     */
    @Override
    ...
    public void setIcons(String[] refs) throws SlickException {
        ByteBuffer[] iconBuff = new ByteBuffer[refs.length];
        for (int i = 0; i < refs.length; i++) {
            ...
            try {
                iconBuff[i] = data.loadImage(ResourceLoader.getResourceAsStream(refs[i]), flip, false, null);
            } catch (Exception e) {
                Log.error(e);
