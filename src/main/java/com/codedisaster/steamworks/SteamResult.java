...
/* JADX INFO: loaded from: game-lib.jar:com/codedisaster/steamworks/SteamResult.class */
public enum SteamResult {
    ...
    static {
        SteamResult[] steamResultArrValues = values();
        int iMax = 0;
        for (SteamResult steamResult : steamResultArrValues) {
            iMax = Math.max(iMax, steamResult.code);
        }
        valuesLookupTable = new SteamResult[iMax + 1];
        for (SteamResult steamResult2 : steamResultArrValues) {
