...
public abstract class LibRocketManager extends LibRocket {
    ...
    public static String convertTexturePath(String str) {
        GameEngine.isInSpace("convertTexturePath for: " + str);
        ...
        if (strUrlDecode.startsWith("assets:")) {
            ...
            GameEngine.isInSpace("convertTexturePath  (basePath:" + z + "):" + strSubstring + " > " + strConvertAbstractPath);
            if (z) {
        ...
        if (strUrlDecode.startsWith(basePath + "assets/gui/")) {
            GameEngine.isInSpace("convertTexturePath already had path:" + strUrlDecode);
            return strUrlDecode;
        }
    ...
    public boolean LoadTexture(int i, String str) throws IOException {
        ...
        if (matcherMatch != null) {
            ...
            GameEngine.isInSpace("alpha=" + strGroup2);
    ...
    public boolean showAlert(DialogData dialogData) {
        if (this.currentAlert != null) {
            GameEngine.isInSpace("AlertPopup already visible closing");
            closeDocument(this.currentAlert.document);
            this.currentAlert = null;
        }
        this.currentAlert = dialogData;
        GameEngine.isInSpace("Showing popup: " + dialogData.title);
        if (!this.isHandlingEvent) {
            update();
        } else {
            GameEngine.isInSpace("insideEvent");
        }
        GameEngine.isInSpace("popup ready..");
        dialogData.document.show(4);
        GameEngine.isInSpace("Popup shown..");
        return true;
    }
    ...
    public boolean showPopup(DialogData dialogData) {
        if (this.currentPopup != null) {
            GameEngine.isInSpace("Popup already visible, cannot show: " + dialogData.document.getMetadata("sourceDocumentId"));
            if (this.currentAlert != null) {
                this.currentAlert.document.pullToFront();
                return false;
            }
    ...
    public void loadCharsetOnChildren(Element element, boolean z) {
        if (element == null) {
            GameEngine.isInSpace("loadCharsetIfNeededOnChildren: root is null");
            return;
        }
    ...
    public boolean closeAlert() {
        ...
        if (dialogData != null) {
            GameEngine.isInSpace("Closing alert");
    ...
    public boolean closePopup() {
        ...
        if (dialogData != null) {
            GameEngine.isInSpace("Closing popup");
            closeDocument(dialogData.document);
