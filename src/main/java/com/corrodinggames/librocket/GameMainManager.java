...
public abstract class GameMainManager {
    ...
    public void showMainMenu() {
        ...
        if (gameEngine != null) {
            ...
            GameEngine.isInSpace("showMainMenu: game is null");
        }
    ...
    public synchronized void endGame(boolean z) {
        GameEngine gameEngine = GameEngine.getInstance();
        if (!z) {
            GameEngine.isInSpace("endGame: queDisconnectAndWait");
            gameEngine.networkEngine.u();
        } else {
            GameEngine.isInSpace("endGame: network disconnect");
        ...
        if (isUIReady()) {
            ...
            if (numInitializeAndroidToGdxMapping != null) {
                ...
                if (Character.isISOControl(c)) {
                    if (c == '\b') {
                        GameEngine.isInSpace("backspace char pressed");
                        this.libRocketManager.processKeyDown(69, 0);
                        this.libRocketManager.processKeyUp(69, 0);
                    } else {
                        GameEngine.isInSpace("keyPressed skipping isISOControl:" + i + " c:" + ((int) c) + " c_print:" + c);
                    }
                } else {
            ...
            if (i == 28 || i == 156) {
                ...
                GameEngine.isInSpace("keyPressed: new line entered");
                ScriptEngine.getInstance().addScriptToQueue("onEnter();");
            }
            if (i == 1) {
    ...
    public boolean openURL(String str) {
        try {
            if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
                try {
                    GameEngine.isInSpace("Opening link from desktop api");
