...
public class Mods extends ScriptContext {
    ...
    public void setModFilter(String str) {
        ...
        if (activeDocument == null) {
            GameEngine.isInSpace("loadMods: No Active Document");
        } else {
    ...
    public void applyModFilter() {
        ...
        if (elementById == null) {
            GameEngine.isInSpace("loadMods: Failed to find modList, wrong page?");
            return;
        }
        ...
        for (Element element : arrayListFindElementsByClassName) {
            ...
            if (modByMapsCount == null) {
                GameEngine.isInSpace("Could not find mod with mod session id: " + iIntValue);
            } else {
                if (strTrim != null) {
                    boolean z2 = false;
    ...
    public void refreshModList() {
        ElementDocument activeDocument = this.libRocket.getActiveDocument();
        if (activeDocument == null) {
            GameEngine.isInSpace("refreshModList: No Active Document");
            return;
        }
        GameEngine.isInSpace("refreshModList");
        if (activeDocument.getElementById("modTemplate") == null) {
            GameEngine.isInSpace("refreshModList: Failed to find modTemplate, wrong page?");
            return;
        }
    ...
    private void _rememberTempModSelection() {
        ElementDocument activeDocument = this.libRocket.getActiveDocument();
        GameEngine gameEngine = GameEngine.getInstance();
        GameEngine.isInSpace("temp save");
    ...
    private void _restoreTempModSelection() {
        ElementDocument activeDocument = this.libRocket.getActiveDocument();
        GameEngine gameEngine = GameEngine.getInstance();
        GameEngine.isInSpace("temp restore");
        for (Element element : activeDocument.findElementsByClassName("modSelection")) {
            String id = element.getId();
    ...
    private void _saveModsCommon(boolean z) throws ConfigParseException {
        ...
        ElementDocument activeDocument = this.libRocket.getActiveDocument();
        GameEngine gameEngine = GameEngine.getInstance();
        GameEngine.isInSpace("savesMods");
        for (Element element : activeDocument.findElementsByClassName("modSelection")) {
            String id = element.getId();
            if (!id.equals("_ID_")) {
                ModInfo modByUuid = gameEngine.modManager.getModByUuid(id);
        ...
        if (z2) {
            GameEngine.isInSpace("mod changes made");
        } else {
            GameEngine.isInSpace("no mod changes made");
        }
    ...
    private void _saveModsMessages(boolean z) throws ConfigParseException {
        ...
        if (gameEngine.networkEngine.B) {
            GameEngine.isInSpace("savesMods: in network game");
        ...
        GameEngine.isInSpace("Errors found");
    }
    ...
    public void reloadModDataAsk() throws ConfigParseException {
        if (GameEngine.getInstance().isGameThreadRunning()) {
            GameEngine.isInSpace("Menu active, reloading without asking");
            reloadModData();
        } else {
