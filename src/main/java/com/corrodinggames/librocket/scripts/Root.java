...
public class Root extends ScriptContext {
    ...
    public void logDebug(String str) {
        GameEngine.isInSpace("ui[debug]: " + str);
    }

    public void logWarn(String str) {
        GameEngine.isInSpace("ui[warn]: " + str);
    }
    ...
    public void showRangeValue(String str, String str2, boolean z) {
        ...
        if (z) {
            string = new Float(attribute).toString();
        } else {
    ...
    public void loadP2PConfig() {
        ...
        if (savedPeerConfig != null) {
            setValueById("p2pLibp2pPeers", savedPeerConfig[0]);
            setValueById("p2pEasyTierPeers", savedPeerConfig[1]);
        }
    }

    public void saveP2PConfig() {
        String valueById = getValueById("p2pLibp2pPeers");
        String valueById2 = getValueById("p2pEasyTierPeers");
        try {
            P2PLobbyService.getInstance().savePeerConfig(valueById == null ? VariableScope.nullOrMissingString : valueById, valueById2 == null ? VariableScope.nullOrMissingString : valueById2);
    ...
    public String getP2PStatusText() {
        return P2PLobbyService.getInstance().getEasyTierStatusText();
    }
    ...
    public void hostP2PStartFromPopup() throws ConfigParseException {
        ...
        String value3 = topmostDocument.getElementById("easyTierPeers").getValue();
        closePopup();
        hostP2PStartWithPasswordAndMods(str, checkbox, value2, value3);
    }

    public void loadP2PHostPopup() {
        String[] savedPeerConfig = P2PLobbyService.getInstance().getSavedPeerConfig();
        if (savedPeerConfig != null) {
            setValueById("libp2pPeers", savedPeerConfig[0]);
            setValueById("easyTierPeers", savedPeerConfig[1]);
        }
    }

    public void hostP2PStartWithPasswordAndMods(String str, boolean z, String str2, String str3) throws ConfigParseException {
        GameEngine gameEngine = GameEngine.getInstance();
        ...
        P2PLobbyService.getInstance().savePeerConfig(str2, str3);
        if (!gameEngine.networkEngine.startServerHosting(false)) {
        ...
        try {
            ...
            P2PLobbyService.getInstance().hostCurrentServer(str2, str3);
            this.libRocket.setDocument("battleroom.rml", null);
        } catch (IOException e) {
    ...
    public void loadConfigAndStartNewSandboxInAdvanced(String str) throws ConfigParseException {
        GameEngine.isInSpace("loadConfigAndStartNewSandboxInAdvanced");
        _loadConfigAndStartNewSandboxCommon(str, false);
        _startAdvancedMode(true);
        GameEngine.isInSpace("editorMode:" + GameEngine.getInstance().isGameStarted);
    }

    private void _loadConfigAndStartNewSandboxCommon(String str, boolean z) {
        if (str.startsWith("saves/")) {
            GameEngine.isInSpace("Starting sandbox from save: " + str);
            loadGame(str.substring("saves/".length()));
        } else {
            GameEngine.isInSpace("Starting sandbox from map: " + str);
    ...
    public void showMapPopup(String str, String str2) {
        ...
        if (elementDocumentCreatePopupWithRML != null) {
            ...
            if (showMapsWithDoc(elementDocumentCreatePopupWithRML)) {
                GameEngine.isInSpace("showMapsWithDoc passed");
    ...
    public void refreshAfterFileImport() {
        GameEngine.isInSpace("refreshAfterFileImport");
    ...
    public boolean showMapsWithDoc(ElementDocument elementDocument) {
        ...
        String str2;
        GameEngine gameEngine = GameEngine.getInstance();
        GameEngine.isInSpace("showMaps");
        if (elementDocument == null) {
            GameEngine.isInSpace("showMaps: elementDocument==null");
            return false;
        }
            ...
            for (int i = 0; i < numAttributes; i++) {
                ...
                if (attributeKey != null) {
                    if (attributeKey.equals("nestedclone") && !attributeValue.equalsIgnoreCase("false")) {
                        GameEngine.isInSpace("nested clone:" + element.getId());
                        ...
                        if (element.getNumChildren() < 1) {
                            GameEngine.isInSpace("child clone failed no children:" + element.getId());
                        }
                        ...
                        if (textVariables != null) {
                            GameEngine.isInSpace("convertTextOnPage:" + attributeKey + ": '" + attributeValue + "' to '" + textVariables + "'");
                            if (attributeKey.equals("_html")) {
                                GameEngine.isInSpace("setting html:" + attributeKey);
                                element.setInnerRML(textVariables);
                            } else {
                                if (attributeKey.startsWith("_")) {
                                    attributeKey = attributeKey.substring("_".length());
                                    GameEngine.isInSpace("converted key to:" + attributeKey);
                                }
    ...
    public void displayServerListRaw(String str, String str2, String str3) {
        ...
        if (activeElementById.getNumChildren() > serverList.size()) {
            for (int numChildren = activeElementById.getNumChildren() - 1; numChildren >= serverList.size(); numChildren--) {
                GameEngine.isInSpace("removing rowIndex:" + numChildren);
                activeElementById.removeChild(activeElementById.getChild(numChildren));
            }
            if (activeElementById.getNumChildren() != serverList.size()) {
        ...
        for (ServerInfo serverInfo : serverList) {
            ...
            if (elementM29clone != null && elementM29clone.hasClassName("serverRowMessage")) {
                GameEngine.isInSpace("removing non rowIndex:" + i);
            ...
            if (elementM29clone != null && elementM29clone.findByClassName("rState") == null) {
                GameEngine.isInSpace("removing non rowIndex with no rState:" + i);
                activeElementById.removeChild(elementM29clone);
                elementM29clone = null;
            }
    ...
    public void trace(String str) {
        GameEngine.isInSpace("Trace:" + str);
    }
    ...
    public void onEnter() {
        ...
        if (topmostDocument == null) {
            GameEngine.isInSpace("onEnter: elementDocument==null");
            return;
        }
    ...
    public void scrollFromFocusedElement(float f) {
        ...
        if (topLevelFocusedElement == null) {
            GameEngine.isInSpace("focusedElement: Not found");
            return;
        }
        ...
        if (chainFromChildElement == null) {
            GameEngine.isInSpace("scrollFromFocusedElement: Failed to find chain");
            return;
        }
        ...
        GameEngine.isInSpace("Found no slider element to offset");
    }
    ...
    public boolean isSliderOrUIElementSelected() {
        ...
        if (topLevelFocusedElement != null) {
            ...
            if (z) {
                GameEngine.isInSpace("Slider element: true");
                return true;
            }
            GameEngine.isInSpace("Slider element: false");
        }
        GameEngine.isInSpace("Slider element: no element focused");
        return false;
    }
    ...
    public void onEscape() {
        ElementDocument topmostDocument = this.libRocket.getTopmostDocument();
        if (topmostDocument == null) {
            GameEngine.isInSpace("onEscape: elementDocument==null");
            return;
        }
        boolean z = false;
    ...
    public void showBattleroom() {
        ...
        if (activeDocument != null && "battleroom.rml".equals(activeDocument.documentPath)) {
            GameEngine.isInSpace("Already on battleroom page");
            z = false;
        }
    ...
    public void event_unicodeEntered() {
        ...
        if (topmostDocument != null) {
            ...
            if (elementFindByClassName != null) {
                ...
                GameEngine.isInSpace("event_unicodeEntered: missing textinput");
                return;
            }
        }
        GameEngine.isInSpace("event_unicodeEntered: missing document");
    }
    ...
    public void openWhitelistedLink(String str) {
        GameEngine.isInSpace("Opening link:" + str);
        if (!str.startsWith("http://corrodinggames.com/") && !str.startsWith("https://corrodinggames.com/") && !str.startsWith("http://corrodinggames.net/") && !str.startsWith("https://corrodinggames.net/")) {
            GameEngine.isInSpace("Not in whitelist");
    ...
    public void writeGameLog(String str) {
        ...
        GameEngine.isInSpace("writeGameLog ready");
        Element activeElementById = this.libRocket.getActiveElementById(str);
        if (activeElementById == null) {
            GameEngine.isInSpace("Failed to find: " + str);
        } else {
            activeElementById.setInnerRML(stringBuffer.toString());
        }
    }
    ...
    public void setPageMinWidthAndHeight(float f, float f2) {
        GameEngine.isInSpace("setPageMinWidthAndHeight(" + f + ", " + f2 + ")");
        ElementDocument activeDocument = this.libRocket.getActiveDocument();
        if (activeDocument == null) {
            GameEngine.isInSpace("setPageMinWidthAndHeight - no page");
            return;
        }
    ...
    public void importFilePopup() {
        PlatformExtension.a(new FileSelectionCallback() { // from class: com.corrodinggames.librocket.scripts.Root.11
            ...
            public void onFileSelected() {
                GameEngine.isInSpace("importFilePopup: onFileSelected");
            }

            @Override // com.corrodinggames.rts.gameFramework.platform.FileSelectionCallback
            public void onCancelled() {
                GameEngine.isInSpace("importFilePopup: onCancelled");
            }
        });
    }
