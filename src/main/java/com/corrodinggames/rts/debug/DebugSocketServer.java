        ...
        new Thread(new Runnable() { // from class: com.corrodinggames.rts.a.a.1
            ...
            public void run() {
                for (String str : DebugSocketServer.scriptsToRun) {
                    GameEngine.isInSpace("Running debug script:" + str);
                    try {
                        ...
                        while (true) {
                            ...
                            if (!strTrim.equals(VariableScope.nullOrMissingString) && !strTrim.startsWith("#")) {
                                GameEngine.isInSpace("Running: " + strTrim);
                                GameEngine.isInSpace("got: " + DebugSocketServer.executeCommand("script " + strTrim).trim());
                            }
                        }
                        bufferedReader.close();
                        fileReader.close();
                        GameEngine.isInSpace("End of:" + str);
                    } catch (IOException e) {
        ...
        while (this.running) {
            try {
                ...
                try {
                    ...
                    GameEngine.isInSpace("Got IOException on debugSocket connection");
                    e.printStackTrace();
                    throw new RuntimeException(e);
                }
