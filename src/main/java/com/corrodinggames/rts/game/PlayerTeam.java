...
public abstract class PlayerTeam extends Serializable implements Comparable<PlayerTeam> {
    ...
    public void updateTeam(float f) {
        ...
        if (this.teamUnitCount > 1000 && this.energy != 0.0d) {
            GameEngine.isInSpace("Warning: anti-lag credits is still: " + this.energy + " (force clearing)");
            this.energy = 0.0d;
        }
    }
    ...
    public static void updateTeamDefeatStatus() {
        if (GameEngine.getInstance().isNetworkGameActive()) {
            GameEngine.isInSpace("Skipping updateAllCachesFromChangedMetadata due to desync risk");
            return;
        }
        for (int i = 0; i < TEAM_NEUTRAL; i++) {
            PlayerTeam playerTeam = teamColorArray[i];
    ...
    public void updateTeamTextures() {
        GameEngine.isInSpace("debugUnitCountByType for team:" + this.teamId);
        ...
        GameEngine.isInSpace("--- Units ---");
        int i2 = 0;
        for (UnitTypeCount unitTypeCount3 : fastArrayList) {
            if (!unitTypeCount3.a.k()) {
                GameEngine.isInSpace(unitTypeCount3.a.getUnitTypeDescriptionShort() + " - count:" + unitTypeCount3.b);
                i2 += unitTypeCount3.b;
            }
        }
        GameEngine.isInSpace("total:" + i2);
        GameEngine.isInSpace("--- Buildings/Ignored in count ---");
        int i3 = 0;
        for (UnitTypeCount unitTypeCount4 : fastArrayList) {
            if (unitTypeCount4.a.k()) {
                GameEngine.isInSpace(unitTypeCount4.a.getUnitTypeDescriptionShort() + " - count:" + unitTypeCount4.b);
                i3 += unitTypeCount4.b;
            }
        }
        GameEngine.isInSpace("total:" + i3);
    }

    public void c(String str) {
        GameEngine.isInSpace("Team(id: " + this.teamId + ", name:" + this.teamName + "):" + str);
    }
