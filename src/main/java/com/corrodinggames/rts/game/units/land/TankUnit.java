...
public class TankUnit extends LandUnit {
    ...
    public void createDustEffect() {
        ...
        while (i <= 1) {
            float f = i == 0 ? -20 : 20;
            gameEngine.effectManager.createMuzzleFlash(this.posX + (Utility.fastCos(this.rotationSpeed + 180.0f + f) * this.speed), this.posY + (Utility.fastSin(this.rotationSpeed + 180.0f + f) * this.speed), this.posZ, this.rotationSpeed + 180.0f, 0);
            i++;
        }
    }
