...
public class TerrainAutotiler {
    ...
    public static void a(EditorTerrainType editorTerrainType, PointF pointF) {
        ...
        if (tileMap == null) {
            GameEngine.isInSpace("setTerrainType called without map loaded");
            return;
        }
        int i = (int) (f * tileMap.tileScaleX);
        int i2 = (int) (f2 * tileMap.tileScaleY);
        if (!tileMap.isInBounds(i, i2)) {
            GameEngine.isInSpace("setTerrainType out of map range");
            return;
        }
        if (tileMap.groundLayer == null) {
            GameEngine.isInSpace("setTerrainType mainLayer missing");
            return;
        }
        ...
        try {
            ...
            if (orCreateTileFromTilesetIndex == null) {
                GameEngine.isInSpace("setTerrainType mapTile==null");
                return;
            }
            ...
            GameEngine.isInSpace("setTerrainType changing " + tileAt.tilesetLocalIndex + " to " + orCreateTileFromTilesetIndex.tilesetLocalIndex + " at:" + i2 + "," + i2);
