...
public class HeavyTankUnit extends LandUnit {
    ...
    public void spawnHeavyTankSmoke() {
        ...
        while (i <= 1) {
            float f2 = i == 0 ? -20 : 20;
            gameEngine.effectManager.createMuzzleFlash(this.posX + (Utility.fastCos(f + 180.0f + f2) * this.speed), this.posY + (Utility.fastSin(f + 180.0f + f2) * this.speed), this.posZ, f + 180.0f, 0);
            i++;
        }
    }
