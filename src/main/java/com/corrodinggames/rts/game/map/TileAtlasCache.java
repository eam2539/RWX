...
public class TileAtlasCache {
    ...
    public TileAtlasCache(float f, boolean z) {
        ...
        if (!z) {
            this.alphaAtlasCache = new TileAtlasCache(f, true);
        }
    }
    ...
    public static boolean hasTransparencyInRect(Texture texture, Rect rect) {
        ...
        if (!texture.k()) {
            GameEngine.isInSpace("hasImageAlpha: Cannot get pixel data for: " + texture.a());
            return true;
        }
        texture.x();
        for (int i = iL; i < iL2; i++) {
            for (int i2 = iM; i2 < iM2; i2++) {
                if (Color.a(texture.a(i2, i)) != 255) {
                    return true;
                }
            }
        }
