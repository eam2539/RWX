...
public abstract class UnitBuildStrategy {
    ...
    public void rebuildUnitMix() {
        ...
        if (this.unitPriorities.size() == 0) {
            GameEngine.isInSpace("AI: rebuildUnitMix: no units in unitMix:" + this.strategyName);
        }
    }
    ...
    public UnitType getRandomUnitTypeWithPrice(UnitMovementType unitMovementType, int i) {
        if (this.unitPriorities.size() == 0) {
            GameEngine.isInSpace("AI: getRandomUnitType: no units in unitMix:" + this.strategyName);
            return null;
        }
        float f = 0.0f;
        int i2 = 0;
        ...
        GameEngine.isInSpace("Did not find getRandomUnit, this should only happen very rarely, name:" + this.strategyName + " unitMix.size:" + this.unitPriorities.size() + " minPrice:" + i + " movementType:" + unitMovementType + " totalUnits:" + i2);
