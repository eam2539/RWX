...
import com.corrodinggames.rts.gameFramework.FormationEngine;
...
import com.corrodinggames.rts.gameFramework.utility.ListOperation;
import com.corrodinggames.rts.gameFramework.utility.ListOperationEntry;
...
public class GameLogic extends GameEngine {
    ...
    public synchronized void init(Context context) {
        ...
        GameEngine.isInSpace("Version:" + getVersionNameWithSuffix());
        ...
        if (isPausedStatic2) {
            ...
            GameEngine.isInSpace("densityScaleRaw: " + this.densityScaleRaw);
            updateDensity(displayMetrics.widthPixels, displayMetrics.heightPixels);
        }
        this.densityScaleRaw *= this.densityScaleMultiplier;
        GameEngine.isInSpace("densityScaleRaw*densityScaleMultiplier: " + this.densityScaleRaw);
        ...
        if (this.settingsEngine.numIncompleteLoadAttempts > 1 || this.settingsEngine.numLoadsSinceRunningGameOrNormalExit > i) {
            ...
            if (this.settingsEngine.newRender && this.settingsEngine.numLoadsSinceRunningGameOrNormalExit > 15) {
                GameEngine.isInSpace("Disabling opengl mode");
                this.settingsEngine.newRender = false;
            }
            GameEngine.isInSpace("starting game in safe mode, numIncompleteLoadAttempts:" + this.settingsEngine.numIncompleteLoadAttempts + " numLoadsSinceRunningGameOrNormalExit:" + this.settingsEngine.numLoadsSinceRunningGameOrNormalExit);
        }
        ...
        if (!this.settingsEngine.save() && isDebugVersionStatic2) {
            GameEngine.isInSpace("starting game in safe mode, failed to save settings");
            this.safeModeReason = "failing to write preferences data";
            this.isSafeMode = true;
        }
        DebugServer.a();
        this.screenScale = getScreenScale();
        GameEngine.isInSpace("densityScale(): " + this.screenScale);
        ...
        if (graphicsEngine != null) {
            isInSpace("init(): using Graphics instance");
            this.graphicsEngine2 = graphicsEngine;
        } else if (gameEngineClass != null) {
            isInSpace("init(): using GraphicsSlick2d");
            try {
        ...
        if (screenSize != null) {
            GameEngine.isInSpace("We have an initial screen size, can do early setup of image buffers");
            ...
            if (GameEngine.isPostProcessingSupported()) {
                ...
                if (!setupPostProcessing()) {
                    GameEngine.isInSpace("Failed to setup postprocessing");
                }
            }
        }
        ...
        GameEngine.isInSpace("Init completed");
    ...
    public void updateDensity(int i, int i2) {
        ...
        GameEngine.isInSpace("defaultViewpointZoomDensity: " + fDistance);
        if (fDistance < 0.5f) {
            fDistance = 0.5f;
        }
        if (fDistance > 3.0f) {
        ...
        GameEngine.isInSpace("defaultViewpointZoomDensity after limit: " + fDistance);
    ...
    public void loadLevel(boolean z, boolean z2, GameMode gameMode)  {
        ...
        try {
            ...
            if (!this.tileMap.isCursorActive) {
                isInSpace("map did not load, returning");
                ...
                for (int i3 = 0; i3 < PlayerTeam.TEAM_NEUTRAL; i3++) {
                    ...
                    if (playerTeamK2 != null) {
                        if (playerTeamK2.fogOfWarData == null) {
                            GameEngine.isInSpace("Fog null for team: " + playerTeamK2.teamId);
                        } else {
                ...
                for (int i6 = 0; i6 < PlayerTeam.TEAM_NEUTRAL; i6++) {
                    ...
                    if (playerTeamK3 != null) {
                        ...
                        if (iIntValue != 1) {
                            ...
                            if (fValueOf == null) {
                                GameEngine.isInSpace("placementLocation==null for team:" + playerTeamK3.teamId);
                            } else {
                                ...
                                if (iIntValue == 2) {
                                    ...
                                    if (customUnitConfigC == null) {
                                        ...
                                        if (customUnitConfigC.canBeBuiltByOrTagsOrLogicAndTagsOrLogicOrTagsAndLogicAndTags) {
            ...
            if (!z2 && (this.missionEngine == null || !this.missionEngine.q)) {
                ...
                isInSpace("there are " + i10 + " units on this map and " + i11 + " trees");
            }
            ...
            if (this.networkEngine.F && this.networkEngine.B) {
                GameEngine.isInSpace("Disabling network for singleplayer");
                this.networkEngine.B = false;
            }
            if (!isDebug()) {
                if (gameMode == GameMode.normalSave) {
                    GameEngine.isInSpace("Not starting replay recording as we are loading a save");
                } else {
            ...
            if (isNetworkServerStatic2) {
                GameEngine.isInSpace("Crashing on allowed map error because automated testing is active");
                throw new RuntimeException(e2);
            }
    ...
    private void selectAnyOnScreenBuilder() {
        ...
        for (BaseUnit baseUnit : BaseUnit.bE) {
            if (baseUnit.team == this.playerTeam && (baseUnit instanceof OrderableUnit) && baseUnit.canMove() && baseUnit.isBuilding() && baseUnit.isAlive() && !baseUnit.u() && !baseUnit.t()) {
                GameEngine.isInSpace("selectAnyOnScreenBuilder: found builder");
                this.gameUI.selectUnit(baseUnit);
                return;
            }
        }
        GameEngine.isInSpace("selectAnyOnScreenBuilder: no builder found");
    }
    ...
    public strictfp void processEvent(float float1, int integer) throws IOException, ConfigParseException {
        ...
        if (isGameThreadRunningStatic && !this.isInGameOrLobby && isDesktop() && Debug.getNativeHeapAllocatedSize() > 209715200L) {
            GameEngine.isInSpace("getNativeHeapAllocatedSize: " + Utility.formatMilliseconds((int)Debug.getNativeHeapAllocatedSize()));
        ...
        if (this.activity.isPaused()) {
            ...
            if (!this.loadNewGame) {
                                ...
                                if (var32.isSurfaceViewReady() && !var32.isContinuousRendering()) {
                                    ...
                                    try {
                                        if (!var32.isContinuousRendering()) {
                                            if (var39 != null) {
                                                if (var39.c()) {
                                                    GameEngine.isInSpace("gameengine draw: bufferedCanvas drawn on");
                                                }
                ...
                if (this.isPinching) {
                    ...
                    if (var36 != null) {
                        GameEngine.isInSpace("gotoNextLevel: Loading next level: " + var36);
                        this.currentMapPath = var36;
                        this.gameUI.messageManager.clear();
                        this.loadLevel(true, false, GameMode.normal);
                    } else {
                        GameEngine.isInSpace("gotoNextLevel: No next level, finishing");
                        this.loadNewGame = false;
                        ...
                        if (var40 != null) {
                            ...
                            GameEngine.isInSpace("gotoNextLevel: Error getInGameActivity==null");
                        }
                    }
                }

                if (!this.isStopped && this.isBenchmarking && !this.someBoolean) {
                    isInSpace("starting method trace");
    ...
    public boolean setupPostProcessing() {
        ...
        if (this.postBaseShader.g || this.postDisplacementShader.g) {
            if (!this.postProcessingFailed) {
                ...
                GameEngine.isInSpace("setupPostprocessing: failed");
                return false;
            }
            return false;
        }
