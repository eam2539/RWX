...
public class TeamColorTexture extends Texture {
    ...
    public static synchronized void C() {
        ...
        try {
            GameEngine.isInSpace("Loading team shaders...");
    ...
    public void c(boolean z2) {
        ...
        if (this.H.A()) {
            GameEngine.isInSpace("Lazy loaded bitmap using errored image: " + this.H.a());
            this.I = this.H;
            return;
        }
        if (z2) {
            try {
                GameEngine.isInSpace("Loading in lazy loaded bitmap:" + this.H.a() + " team:" + this.E);
            } catch (OutOfMemoryError e) {
                GameEngine.isInSpace("Colouring failed with OOM");
        ...
        if (dA2 > 1.0d) {
            GameEngine.isInSpace((this.F == ColorMode.pureGreen ? "Standard " : "Hue ") + "Colouring took:" + PerformanceProfiler.a(dA2) + " (" + PerformanceProfiler.a(dA) + ")");
        }
    ...
    public Bitmap b() {
        if (this.C && !GameEngine.areShadersSupported()) {
            GameEngine.isInSpace("Team shader coloring now disabled. Recoloring image: " + this.H.a());
            this.B = false;
            this.C = false;
