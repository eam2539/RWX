...
public class SettingsEngine {
    ...
    public boolean saveToFileSystem() {
        ...
        GameEngine.isInSpace("Saving settings to: " + file.getAbsolutePath());
        try {
    ...
    public void loadFromFileSystem() {
        ...
        if (!file.exists()) {
            GameEngine.isInSpace("preferences not found, not loading (" + preferencesPath + ")");
            return;
        }
        GameEngine.isInSpace("loadFromFileSystem filepath: " + file.getAbsolutePath());
        try {
            ...
            while (it.hasNext()) {
                ...
                if (GameEngine.isDebugVersionStatic2) {
                    GameEngine.isInSpace(str + "= " + string);
                }
                if (string != null && !VariableScope.nullOrMissingString.equals(string)) {
                    if (str.startsWith("key.")) {
                        ...
                        GameEngine.isInSpace("loading keybind:" + strReplace);
    ...
    private SettingsEngine(Context context) {
        ...
        for (Field field : getClass().getFields()) {
            ...
            if (this.settingFields.get(name) != null) {
                GameEngine.isInSpace("SettingsEngine: fields: " + name + " already exists");
            }
            if (GameEngine.isDebugVersionStatic2) {
                GameEngine.isInSpace("SettingsEngine: field:" + name);
            }
    ...
    public boolean loadMainExternalFolder(boolean z) {
        ...
        GameEngine.isInSpace("loadMainExternalFolder..");
        GameEngine gameEngine = GameEngine.getInstance();
        if (z && gameEngine.isCustomGameMode()) {
            GameEngine.isInSpace("Not loading due to extra safe mode");
            return false;
        }
        ...
        if (str == null) {
            GameEngine.isInSpace("No external folder set");
            return false;
        }
        GameEngine.isInSpace("External saf link: " + str);
        GameEngine.isInSpace("External saf shown path: " + str2);
        GameEngine.isInSpace("External saf extra: " + str3);
        try {
            Uri uri = Uri.parse(str);
            boolean z2 = false;
            String str4 = null;
            String strA = SafFileLoader.a(uri, true);
            GameEngine.isInSpace("safVirualPathBase: " + strA);
            if (strA == null) {
                GameEngine.isInSpace("createSAFLink failed for uri: " + uri);
                z2 = true;
            } else {
                str4 = strA + str3;
                GameEngine.isInSpace("safVirualPath: " + str4);
                if (!FileHelper.isDirectoryNonZip(str4)) {
                    GameEngine.isInSpace("isDirectory failed for: " + str4);
                    z2 = true;
                }
            }
            if (z2) {
                GameEngine.isInSpace("error for uri: " + uri);
            ...
            GameEngine.isInSpace("Using external path");
