...
import com.corrodinggames.rts.game.units.UnitSpawner;
...
public class Command {
    ...
    public void executeCommand() {
        ...
        if (this.isSystemAction) {
            if (this.gameSpeedChange != 0.0f) {
                GameEngine.isInSpace("issueCommand: changeStepRate:" + this.gameSpeedChange);
                gameEngine.networkEngine.applyChangedSetup(this.gameSpeedChange, "command");
                return;
            }
            if (this.systemActionType != 0) {
                GameEngine.isInSpace("system action:" + this.systemActionType);
                if (this.systemActionType == 1) {
                    GameEngine.isInSpace("new DebugDesyncDetector");
                    new UnitSpawner(false).setUnitTeam(PlayerTeam.TEAM_ALL);
                    return;
                }
                if (this.systemActionType == 2) {
                    GameEngine.isInSpace("new DebugDesyncDetector (stress test)");
                ...
                if (this.systemActionType == 100) {
                    GameEngine.isInSpace("team surrender");
                    if (this.team == null) {
                        GameEngine.isInSpace("team not found");
                        return;
                    }
                ...
                GameEngine.isInSpace("issueCommand: unknown system action:" + this.systemActionType);
                return;
            }
            GameEngine.isInSpace("issueCommand: Null System action");
            return;
        }
        if (this.sourceTeam != null) {
    ...
    public boolean prepareAndValidateCommand() {
        ...
        if (GameEngine.getInstance().getVersionCode(true) < 127 && this.unitCommand != null && this.unitCommand.getCommandType() == UnitCommandType.build && (buildUnitType = this.unitCommand.getBuildUnitType()) != null && (baseUnitFindTurretPosition = BaseUnit.findTurretPosition(buildUnitType)) != null && !(baseUnitFindTurretPosition instanceof OrderableUnit)) {
            GameEngine.isInSpace("Rejecting non OrderableUnit build order: " + buildUnitType.getUnitTypeDescriptionShort());
            return false;
        }
        if (this.unitCommand != null && this.unitCommand.isForceMove) {
            GameEngine.isInSpace("Rejecting waypoint with addedByAction true");
            return false;
        }
        return true;
    }
