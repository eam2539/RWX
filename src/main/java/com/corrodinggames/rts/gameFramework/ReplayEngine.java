...
public class ReplayEngine {
    ...
    public static void a(String str) {
        GameEngine.isInSpace("Replay: " + str);
    }
    ...
    public boolean a(String str, File file) {
        ...
        try {
            if (file.isDirectory()) {
                GameEngine.isInSpace("File is a directory: " + file.getAbsolutePath());
                GameEngine.isInSpace("Cannot load replay: Target is a folder, instead of a file");
                gameEngine.alert("Cannot load replay: Target is a folder, instead of a file", 1);
                return false;
            }
            this.C = FileHelper.openFile(file);
            if (this.C == null) {
                GameEngine.isInSpace("Cannot load replay: Failed to read replay file");
            ...
            if (!utf.equals("rustedWarfareReplay")) {
                GameEngine.isInSpace("Header is not correct:" + utf);
                GameEngine.isInSpace("Cannot load replay: File is missing header (check if this file is a replay)");
    ...
    public boolean h() throws IOException {
        ...
        if ("es".equals(strStartBlockAndGetName)) {
            ...
            if (!this.n) {
                ...
                if (z) {
                    GameEngine.isInSpace("replay: -trace checksum-");
                } else {
                    GameEngine.isInSpace("replay: -long checksum-");
                }
        ...
        if ("resync".equals(strStartBlockAndGetName)) {
            ...
            GameEngine.isInSpace("Loading resync from replay");
        ...
        if ("end".equals(strStartBlockAndGetName)) {
            ...
            this.F.d("end");
            GameEngine.isInSpace("number of replay commands issued:" + this.z);
            return false;
        }
            ...
            if (this.w != null) {
                if (this.n) {
                    ...
                    if (b && this.w != null && this.x != this.w) {
                        this.x = this.w;
                        GameEngine.isInSpace("replay: upcoming in " + (this.w.a - gameEngine.currentTick) + " command:" + (this.w.e != null));
                    }
                    if (this.w.b && this.z == 0) {
                        GameEngine.isInSpace("updateGameFrame: replay: Skipping wait on first resync without commands to avoid delay");
                        this.w = null;
                            ...
                            if (gameEngine.currentTick > this.w.a) {
                                ...
                                if (d) {
                                    if (this.w.e.sourceTeam == null) {
                                        GameEngine.isInSpace("Precommand Team: commandingPlayer==null");
                                        if (this.w.e.team != null) {
                                            GameEngine.isInSpace("Precommand Team id:" + this.w.e.team.teamId + " credits:" + this.w.e.team.credits);
                                        }
                                    } else {
                                        GameEngine.isInSpace("Precommand Team id:" + this.w.e.sourceTeam.teamId + " credits:" + this.w.e.sourceTeam.credits + " count:" + this.w.e.sourceTeam.getTeamUnitCountInt() + " max:" + this.w.e.sourceTeam.getTeamBuildingCountInt());
                                    }
                                }
                                ...
                                if (d) {
                                    if (this.w.e.sourceTeam != null) {
                                        GameEngine.isInSpace("Postcommand credits:" + this.w.e.sourceTeam.credits + " count:" + this.w.e.sourceTeam.getTeamUnitCountInt() + " max:" + this.w.e.sourceTeam.getTeamBuildingCountInt());
                                    } else if (this.w.e.team != null) {
                                        GameEngine.isInSpace("Postcommand Team id:" + this.w.e.team.teamId + " credits:" + this.w.e.team.credits);
                                    }
                                }
                                this.z++;
                            }
    ...
    public void e(String str) {
        GameEngine.isInSpace("ReplayEngine deleteGame: " + str);
        String strMapPath = FileHelper.mapPath(str);
        if (strMapPath.contains("\\") || strMapPath.contains("/")) {
            GameEngine.isInSpace("Cannot get replay with path: " + str);
            return;
        }
        File fileA = a(str, true);
        GameEngine.isInSpace("ReplayEngine path: " + fileA.getAbsolutePath());
        if (!fileA.exists()) {
            GameEngine.isInSpace("ReplayEngine deleteGame: file doesn't exist");
        }
        if (!FileHelper.deleteDirectory(fileA)) {
            GameEngine.isInSpace("ReplayEngine deleteGame: failed to delete: " + fileA.getAbsolutePath());
        }
