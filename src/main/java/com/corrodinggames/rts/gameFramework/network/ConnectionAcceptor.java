...
public class ConnectionAcceptor implements Runnable {
    ...
    public boolean isIpAllowed(InetAddress inetAddress, boolean z) {
        if (inetAddress == null) {
            GameEngine.isInSpace("isIpAllowed: inetAddress==null");
            return true;
        }
        if (!b) {
            return true;
        ...
        synchronized (this.l) {
                ...
                if (inetAddress.equals(connectionAttemptTracker.a)) {
                    ...
                    if (connectionAttemptTracker.b > i) {
                        if (!connectionAttemptTracker.c) {
                            ...
                            GameEngine.isInSpace("DOS: Too many attempts:" + connectionAttemptTracker.b + " ip:" + inetAddress.toString());
                        }
                        if (connectionAttemptTracker.b > 300 && !connectionAttemptTracker.d) {
                            connectionAttemptTracker.d = true;
                            GameEngine.isInSpace("DOS: Excessive attempts:" + connectionAttemptTracker.b + " ip:" + inetAddress.toString());
                        }
                        return false;
                    }
                    z2 = true;
            ...
            if (this.acceptedCount > 500) {
                if (!this.p) {
                    this.p = true;
                    GameEngine.isInSpace("DOS: Too many unique attempts: " + this.acceptedCount + ". udp:" + this.isUdp);
                    return false;
                }
                return false;
            }
            ...
            if (i2 > i4) {
                if (!this.q) {
                    this.q = true;
                    GameEngine.isInSpace("DOS: Too open connections from same ip:" + inetAddress.toString() + " (count:" + i2 + ") max:" + i4);
                    return false;
                }
                return false;
            }
            if (i3 > 300) {
                if (!this.o) {
                    this.o = true;
                    GameEngine.isInSpace("DOS: Too open connections locking down:" + inetAddress.toString() + " (count:" + i3 + ")");
                    return false;
                }
                return false;
            }
    ...
    public void startSocket(boolean z) throws IOException {
        ...
        reliableServerSocket.setAcceptancePolicy(new AcceptFilter() { // from class: com.corrodinggames.rts.gameFramework.j.ao.1
            ...
            public boolean shouldAcceptPeer(SocketAddress socketAddress) {
                ...
                GameEngine.isInSpace("AcceptFilter: Unhandled SocketAddress type:" + socketAddress.getClass().getName());
                return true;
            }
        });
        ...
        while (this.isRunning) {
            try {
                ...
                try {
                    ...
                    GameEngine.isInSpace("Got IOException on new player connection");
                    e.printStackTrace();
                }
                ...
                if (this.isRunning) {
                    ...
                    i++;
                    GameEngine.isInSpace("ServerSocket-accept(" + (this.isUdp ? "udp" : "tcp") + ") failed: " + e2.getMessage() + " (closed:" + this.serverSocket.isClosed() + ")");
                    if (i > 100) {
                        GameEngine.isInSpace("Too many server socket fails");
                        stop();
                        return;
                    }
                    try {
                    ...
                    GameEngine.isInSpace("ServerSocket-accept(" + (this.isUdp ? "udp" : "tcp") + "): Got expected IOException after closed socket");
                    return;
                }
            }
        }
