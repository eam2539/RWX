...
public class Path {
    ...
    public void a(GameOutputStream gameOutputStream) throws IOException {
        ...
        if (this.x.size() != 0) {
            ...
            for (int i2 = 1; i2 < this.x.size(); i2++) {
                ...
                if (z) {
                    GameEngine.isInSpace("writeOutCompressedPath: out of range:" + i3 + "," + i4);
                    i = 128;
                } else {
                    i = i3 + 1 + ((i4 + 1) << 2);
                }
    ...
    public void a(GameInputStream gameInputStream) throws IOException {
        ...
        if (i > 160000 || i < 0) {
            GameEngine.isInSpace("readInCompressedPath: Path too big at:" + i);
            i = -1;
        }
        if (i != -1) {
            this.u = true;
            ...
            for (int i2 = 1; i2 < i; i2++) {
                ...
                if (b < 128) {
                    ...
                    if (Utility.abs(i3) > 1 || Utility.abs(i4) > 1) {
                        GameEngine.isInSpace("readInCompressedPath: out of range but shouldn't be:" + i3 + "," + i4 + " for: " + ((int) b));
                    }
                    shortValue = (short) (shortValue + i3);
                    shortValue2 = (short) (shortValue2 + i4);
                    this.x.add(new PathPoint(shortValue, shortValue2));
                } else {
                    GameEngine.isInSpace("readInCompressedPath: out of range unpack:" + ((int) shortValue) + "," + ((int) shortValue2));
