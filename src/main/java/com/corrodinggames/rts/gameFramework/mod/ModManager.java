...
public class ModManager {
    ...
    public void loadModSelection() {
        boolean z = false;
        GameEngine.isInSpace("Loading mod selection");
        for (String str : GameEngine.getInstance().settingsEngine.modSettings.split(",")) {
            String[] strArrSplit = str.split("\\|");
            if (strArrSplit.length != 3) {
                GameEngine.isInSpace("loadSelection: wrong count (" + strArrSplit.length + "):" + str);
            } else {
                ...
                if (str4.equals("enabled")) {
                    ...
                    GameEngine.isInSpace("loadSelection: Unknown option:" + str);
                }
                ModInfo modByUuid = getModByUuid(str3);
                if (modByUuid == null) {
                    GameEngine.isInSpace("loadSelection: Did not find mod in settings:" + str2);
                } else {
    ...
    public void loadModsFromDir(String str, boolean z, boolean z2) throws IOException {
        GameEngine.isInSpace("loading mod custom units at:" + str);
    ...
    public ArrayList getAllUnitBlueprintsFromEnabledMods() {
        ArrayList arrayList = new ArrayList();
        for (ModInfo modInfo : this.mods) {
    ...
    public ArrayList loadAllMods() {
        try {
            ...
            if (disabledSteamEngineA != null) {
                ...
                GameEngine.isInSpace("getAllModList: SteamEngine==null");
            }
            ...
            if (!FileHelper.isDirectoryNonZip(builtinModsFolderName)) {
                GameEngine.isInSpace("Modded Custom '" + builtinModsFolderName + "' directory not found");
            } else {
            ...
            if (!FileHelper.isDirectoryNonZip(builtinModsEnabledFolderName)) {
                GameEngine.isInSpace("Modded Custom '" + builtinModsEnabledFolderName + "' directory not found");
            } else {
                loadModsFromDir(builtinModsEnabledFolderName, true, true);
            }
            for (ModInfo modInfo2 : this.mods) {
                if (!modInfo2.found) {
                    GameEngine.isInSpace("Removing mod no longer found on system: " + modInfo2.getDisplayTitle());
                    removeMod(modInfo2);
                }
            }
            GameEngine.isInSpace("========= Mods ===========");
            GameEngine.isInSpace("Number of mods:" + this.mods.size());
            for (ModInfo mod : this.mods) {
                GameEngine.isInSpace("Mod: '" + mod.getDisplayTitle());
            }
            GameEngine.isInSpace("================================");
            GameEngine gameEngine = GameEngine.getInstance();
            if (gameEngine.settingsEngine.lastModCount == -1 || gameEngine.settingsEngine.modSettingsVersion < 1) {
                GameEngine.isInSpace("Disabling all new mods for first/new load");
                for (ModInfo mod : this.mods) {
                    mod.disabled = true;
                }
                saveModSelection();
                ...
                GameEngine.isInSpace("Too many new mods found, not enabling new mods");
                GameEngine.isInSpace("Number of mods:" + this.mods.size() + " vs " + gameEngine.settingsEngine.lastModCount);
        ...
        if (!z2) {
            for (ModInfo modInfo : this.mods) {
                if (modInfo.firstError != null) {
                    GameEngine.isInSpace("re-enabling mod: " + modInfo.getDisplayTitle());
                }
    ...
    public void triggerStatisticsUpdate() {
        ...
        if (gameEngine.missionEngine2 != null) {
            ...
            GameEngine.isInSpace("No active callbacks");
        }
    }
    ...
    public String[] addExtraMapsForPath(String[] strArr, String str) {
        ...
        GameEngine.isInSpace("addExtraMapsForPath: " + str);
        ArrayList arrayList = new ArrayList();
        if (strArr != null) {
    ...
    public ArrayList<LegacyDisabledMod> getLegacyDisabledModsByPath(String str) {
        ...
        for (LegacyDisabledMod legacyDisabledMod : this.invalidMods) {
            ...
            if (z) {
                GameEngine.isInSpace("Adding extra map:" + legacyDisabledMod.modId);
    ...
    public void addInvalidMod(String str, ModInfo modInfo) {
        ...
        if (strSubstring.startsWith(str2)) {
            ...
            if (strMapPath.startsWith(str2)) {
                ...
                GameEngine.isInSpace("Mod path:" + modInfo.sourceFolder + " in map path without tag:" + strSubstring);
            } else {
            ...
            if (strArrSplit.length >= 2) {
                for (int length = strArrSplit.length - 2; length >= 0; length--) {
                    ...
                    if (str2.startsWith("MOD|")) {
                        ...
                        if (modByUuid == null) {
                            GameEngine.isInSpace("getLinkedModForFile: Failed to find mod with hash:" + strSubstring);
                            return null;
                        }
                        return modByUuid;
                    }
