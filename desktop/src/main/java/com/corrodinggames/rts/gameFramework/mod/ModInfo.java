...
public class ModInfo extends ModMetadata implements Comparable<ModInfo> {
    ...
    /* JADX INFO: renamed from: Q */
    ArrayList<String> unitBlueprints;
    ...
    public void addError(String str) {
        ...
        if (this.firstError == null) {
            ...
            GameEngine.log("Disabling mod due to error: " + getPaddedTitle() + " path:" + getAbsolutePath());
            this.firstError = str;
        }
        this.errors.add(str);
    }
        ...
        if (!CacheManager.existsInCache("mods-info", this.sourceFolder + "/mod-info.txt")) {
            ...
            if (strArrListDirCached != null && strArrListDirCached.length == 1) {
                ...
                if (FileHelper.isDirectoryNonZip(str2) && FileHelper.fileExists(str3)) {
                    GameEngine.log("Changing mod sourceFolder to:" + str2);
    ...
    public IniFile getModInfoFile() {
        ...
        if (this.sourceFolder == null) {
            GameEngine.log("No source yet for mod: " + this.dirName);
            return null;
        }
        ...
        try {
            ...
            if (inputStreamOpenFileByPath == null) {
                GameEngine.log("No mod info for: " + this.dirName + " at " + str);
                return null;
            }
            try {
                return new IniFile(inputStreamOpenFileByPath, str);
            } catch (IOException e) {
                GameEngine.log("Error loading mod info for: " + this.dirName + " at " + str);
                e.printStackTrace();
    ...
    public void logMemoryUsage() {
        GameEngine.log("Mod: '" + getDisplayTitle() + "' - Memory use:" + getMemoryUsageString() + " " + (isEnabled() ? VariableScope.nullOrMissingString : " (disabled)"));
    }

    /* JADX INFO: renamed from: u */
    public boolean delete() {
        GameEngine.log("Trying to delete mod: '" + getDisplayTitle() + "'");
        String path = getPath();
        ...
        if (!FileHelper.fileExists(file.getAbsolutePath())) {
            GameEngine.log("Mod: '" + getDisplayTitle() + "' - cannot delete: Not a file");
            return false;
        }
        boolean zDeleteDirectory = FileHelper.deleteDirectory(file);
        GameEngine.log("Delete result: " + zDeleteDirectory);
