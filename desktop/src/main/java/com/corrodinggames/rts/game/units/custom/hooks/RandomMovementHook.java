    ...
    public void b(CustomUnit customUnit, float f) {
        ...
        if (customUnit.isSlidingMovement()) {
            if (Utility.abs(customUnit.velocityX) < this.c) {
                customUnit.velocityX += Utility.getDeterministicRandomFloatForUnit(customUnit, -this.b, this.b, 1);
            }
            if (Utility.abs(customUnit.velocityY) < this.c) {
                customUnit.velocityY += Utility.getDeterministicRandomFloatForUnit(customUnit, -this.b, this.b, 2);
            }
        } else {
            if (Utility.abs(customUnit.rotation) < this.c) {
                customUnit.rotation += Utility.getDeterministicRandomFloatForUnit(customUnit, -this.b, this.b, 1);
            }
            customUnit.rotationSpeed += Utility.getDeterministicRandomFloatForUnit(customUnit, -1.0f, 1.0f, 2);
        }
        if (this.d > 0) {
            if (customUnit.posY > gameEngine.tileMap.getWorldHeight() - this.d) {
                customUnit.velocityY -= Utility.getDeterministicRandomFloatForUnit(customUnit, 0.0f, this.b * 0.25f, 10);
            }
            if (customUnit.posY < this.d) {
                customUnit.velocityY += Utility.getDeterministicRandomFloatForUnit(customUnit, 0.0f, this.b * 0.25f, 11);
            }
            if (customUnit.posX > gameEngine.tileMap.getWorldWidth() - this.d) {
                customUnit.velocityX -= Utility.getDeterministicRandomFloatForUnit(customUnit, 0.0f, this.b * 0.25f, 12);
            }
            if (customUnit.posX < this.d) {
                customUnit.velocityX += Utility.getDeterministicRandomFloatForUnit(customUnit, 0.0f, this.b * 0.25f, 13);
            }
        }
