...
class AndroidPlatformBridge(context: Context) : PlatformBridge {
    ...
    private fun createAppMetadata(context: Context): AppMetadata {
        ...
        val signature = runCatching {
            val signatureInfo = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        ...
        return AppMetadata(
            ...
            versionName = packageInfo?.versionName ?: AppMetadata.DEFAULT_VERSION_NAME,
            versionCode = packageInfo?.versionCode ?: AppMetadata.DEFAULT_VERSION_CODE,
            signature = signature,
        )
    }
}
...
private class AndroidPlatformAudio(context: Context) : PlatformAudio {
    ...
    override fun registerBuiltInSound(id: String, name: String): Boolean = synchronized(lock) {
        ...
        val resourceId = appContext.resources.getIdentifier(name, "raw", appContext.packageName)
        if (resourceId == 0) return@synchronized false
...
internal fun androidCrashEnvironment(
    ...
    "app.versionName" to (appMetadata?.versionName ?: AppMetadata.DEFAULT_VERSION_NAME),
    "app.versionCode" to (appMetadata?.versionCode ?: AppMetadata.DEFAULT_VERSION_CODE).toString(),
