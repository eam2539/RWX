...
abstract class GameSession {
    ...
    open fun requestSaveGame(name: String) {
        synchronized(gameLock) {
            ...
            runCatching {
                engine.gameSaver.updateAutosave(saveName, false)
    ...
    open fun captureMemorySnapshot(): GameMemorySnapshot? {
        synchronized(gameLock) {
            ...
            return runCatching {
                ...
                engine.gameSaver.createTempFile(output)
    ...
    protected open fun loadMap(engine: GameEngine, mapPath: String) {
        ...
        if (saveName != null) {
            check(engine.gameSaver.performAutosave(saveName, true)) {
    ...
    protected open fun loadMemorySnapshot(engine: GameEngine, snapshot: GameMemorySnapshot) {
        ...
        engine.isStopped = false
        engine.isPaused = false
        check(engine.gameSaver.writeSaveToStream(GameInputStream(snapshot.saveBytes), false, false, false)) {
