...
public abstract class OrderableUnit extends UnitBase {
            ...
            if (this.attackTarget != null) {
                ...
                if (fDistanceSq < engagementRange * engagementRange || z2) {
                    ...
                    for (int i3 = 0; i3 < techLevel; i3++) {
                        ...
                        if (baseUnit2 != null) {
                            if (shouldProbeFireDecision(baseUnit2)) {
                                logFireTargetingLoopProbe("advance-loop", baseUnit2, i3, z);
                            }
                            if (u(i3) && unitMovementData2.rotation == 0.0f) {
                                z = true;
                                if (shouldProbeFireDecision(baseUnit2)) {
                                    logFireTargetingLoopProbe("sets-global-charged", baseUnit2, i3, z);
                                }
                            }
                            if (u(i3)) {
                                advanceMovementLevelTowardTarget(f, baseUnit2, i3);
                            } else if (shouldProbeFireDecision(baseUnit2)) {
                                logFireTargetingLoopProbe("skip-inactive", baseUnit2, i3, z);
                            }
                        }
                    }
    ...
    public boolean advanceMovementLevelTowardTarget(float f, BaseUnit baseUnit, int i) {
        ...
        boolean zS = s(i);
        boolean z = false;
        boolean probeFireDecision = shouldProbeFireDecision(baseUnit);
        Boolean canFireAtTarget = null;
        if (probeFireDecision) {
            logFireDecisionProbe("enter", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
        }
        if (zS) {
            ...
            z = true;
            if (probeFireDecision) {
                logFireDecisionProbe("spooled", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
            }
        }
        if (unitMovementData.rotation == 0.0f && r(i)) {
            canFireAtTarget = Boolean.valueOf(a(i, baseUnit, false, false));
            if (probeFireDecision) {
                logFireDecisionProbe("gate", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
            }
            if (!canFireAtTarget.booleanValue()) {
                unitMovementData.rotation = -10.0f;
                if (probeFireDecision) {
                    logFireDecisionProbe("blocked", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
                }
                return false;
            }
            if (!zS) {
                ...
                if (probeFireDecision) {
                    logFireDecisionProbe("charged", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
                }
            }
            if (z) {
                if (probeFireDecision) {
                    logFireDecisionProbe("fire-before", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
                }
                ...
                if (probeFireDecision) {
                    logFireDecisionProbe("fire-after", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
                }
                return true;
            }
            if (probeFireDecision) {
                logFireDecisionProbe("wait", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
            }
            return false;
        }
        if (probeFireDecision) {
            logFireDecisionProbe("skip-rotation", baseUnit, i, linkedTurretIndex, zS, z, canFireAtTarget, f);
        }
        return false;
    }

    private boolean shouldProbeFireDecision(BaseUnit target) {
        if (!"1".equals(System.getenv("RWX_PROBE_FIRE_DECISION"))) {
            return false;
        }
        GameEngine gameEngine = GameEngine.getInstance();
        if (gameEngine == null) {
        ...
        String startValue = System.getenv("RWX_PROBE_FIRE_START_TICK");
        String endValue = System.getenv("RWX_PROBE_FIRE_END_TICK");
        if (startValue != null || endValue != null) {
            try {
                int startTick = startValue == null ? Integer.MIN_VALUE : Integer.parseInt(startValue);
                int endTick = endValue == null ? Integer.MAX_VALUE : Integer.parseInt(endValue);
                if (gameEngine.currentTick < startTick || gameEngine.currentTick > endTick) {
                    return false;
                }
        ...
        String idsValue = System.getenv("RWX_PROBE_FIRE_IDS");
        if (idsValue != null && !matchesProbeId(idsValue, this.objectId)) {
            return false;
        }
        String targetIdsValue = System.getenv("RWX_PROBE_FIRE_TARGET_IDS");
        if (targetIdsValue != null && (target == null || !matchesProbeId(targetIdsValue, target.objectId))) {
            return false;
        }
        return true;
    }

    private static boolean matchesProbeId(String idsValue, long objectId) {
        String[] ids = idsValue.split(",");
        for (String id : ids) {
            try {
                if (Long.parseLong(id.trim()) == objectId) {
                    return true;
                }
            } catch (NumberFormatException ignored) {
            }
    ...
    private void logFireDecisionProbe(
            String stage,
            BaseUnit target,
            int turretIndex,
            int linkedTurretIndex,
            boolean spoolsWhileCooling,
            boolean charged,
            Boolean canFireAtTarget,
            float delta) {
        GameEngine gameEngine = GameEngine.getInstance();
        UnitMovementData movement = this.movementLevels[turretIndex];
        UnitMovementData linkedMovement =
                linkedTurretIndex == -1 ? null : this.movementLevels[linkedTurretIndex];
        System.out.println(
                "FIRE_DECISION tick=" + (gameEngine == null ? -1 : gameEngine.currentTick) +
                        " stage=" + stage +
                        " unit=" + this.objectId +
                        ":" + r().getUnitTypeDescriptionShort() +
                        ":team=" + (this.team == null ? "null" : this.team.teamId) +
                        " turret=" + turretIndex +
                        " linked=" + linkedTurretIndex +
                        " target=" + (target == null ? "null" : target.objectId) +
                        ":" + (target == null ? "null" : target.r().getUnitTypeDescriptionShort()) +
                        ":team=" + (target == null || target.team == null ? "null" : target.team.teamId) +
                        ":hp=" + (target == null ? "null" : target.currentHealth) +
                        ":pos=" + (target == null ? "null" : target.posX + "," + target.posY) +
                        " delta=" + delta +
                        ...
                        " targetUnit=" + (movement.targetUnit == null ? "null" : movement.targetUnit.objectId) +
                        " linkedActive=" + (linkedMovement == null ? "null" : linkedMovement.isActive) +
                        " linkedRot=" + (linkedMovement == null ? "null" : linkedMovement.rotation) +
                        " linkedSpeed=" + (linkedMovement == null ? "null" : linkedMovement.speed) +
                        " zS=" + spoolsWhileCooling +
                        " charged=" + charged +
                        " canFire=" + canFireAtTarget +
                        " r=" + r(turretIndex) +
                        " u=" + u(turretIndex) +
                        " e=" + e(turretIndex) +
                        " b=" + b(turretIndex) +
                        ...
                        " unitFlags4=" + this.unitFlags4 +
                        " pos=" + this.posX + "," + this.posY +
                        " bodyRot=" + this.rotationSpeed);
    }

    private void logFireTargetingLoopProbe(String stage, BaseUnit target, int turretIndex, boolean globalCharged) {
        GameEngine gameEngine = GameEngine.getInstance();
        UnitMovementData movement = this.movementLevels[turretIndex];
        System.out.println(
                "FIRE_TARGETING tick=" + (gameEngine == null ? -1 : gameEngine.currentTick) +
                        " stage=" + stage +
                        " unit=" + this.objectId +
                        ":" + r().getUnitTypeDescriptionShort() +
                        ":team=" + (this.team == null ? "null" : this.team.teamId) +
                        ...
                        " targetY=" + movement.targetY +
                        " targetUnit=" + (movement.targetUnit == null ? "null" : movement.targetUnit.objectId) +
                        " linked=" + getLinkedTurretIndex(turretIndex) +
                        " canFire=" + a(turretIndex, target, false, false) +
                        " x=" + x(turretIndex) +
                        " targetTurretRotation=" + this.targetTurretRotation +
                        " turretRotation=" + this.turretRotation +
                        " unitFlags4=" + this.unitFlags4 +
    ...
    public void M(int i) {
        ...
        if (shouldProbeMovementRandom()) {
            logMovementRandomProbe(i, unitMovementData, randomRange);
        }
    }

    private boolean shouldProbeMovementRandom() {
        if (!"1".equals(System.getenv("RWX_PROBE_MOVEMENT_RANDOM"))) {
            return false;
        }
        GameEngine gameEngine = GameEngine.getInstance();
        if (gameEngine == null) {
        ...
        String startValue = System.getenv("RWX_PROBE_MOVEMENT_RANDOM_START_TICK");
        String endValue = System.getenv("RWX_PROBE_MOVEMENT_RANDOM_END_TICK");
        if (startValue != null || endValue != null) {
            try {
                int startTick = startValue == null ? Integer.MIN_VALUE : Integer.parseInt(startValue);
                int endTick = endValue == null ? Integer.MAX_VALUE : Integer.parseInt(endValue);
        ...
        String idsValue = System.getenv("RWX_PROBE_MOVEMENT_RANDOM_IDS");
        return idsValue == null || matchesProbeId(idsValue, this.objectId);
    }

    private void logMovementRandomProbe(int turretIndex, UnitMovementData movement, float randomRange) {
        GameEngine gameEngine = GameEngine.getInstance();
        System.out.println(
                "MOVEMENT_RANDOM tick=" + (gameEngine == null ? -1 : gameEngine.currentTick) +
                        " seed=" + (gameEngine == null ? -1 : gameEngine.globalSeed) +
                        " unit=" + this.objectId +
                        ":" + r().getUnitTypeDescriptionShort() +
                        ":team=" + (this.team == null ? "null" : this.team.teamId) +
                        " turret=" + turretIndex +
                        " target=" + (this.attackTarget == null ? "null" : this.attackTarget.objectId) +
                        ":" + (this.attackTarget == null ? "null" : this.attackTarget.r().getUnitTypeDescriptionShort()) +
                        ":team=" + (this.attackTarget == null || this.attackTarget.team == null ? "null" : this.attackTarget.team.teamId) +
                        ":radius=" + (this.attackTarget == null ? "null" : this.attackTarget.radius) +
                        " randomRange=" + randomRange +
                        " flags4=" + this.unitFlags4 +
                        " pos=" + this.posX + ":" + Float.floatToRawIntBits(this.posX) +
                        "," + this.posY + ":" + Float.floatToRawIntBits(this.posY) +
                        " h=" + movement.h + ":" + Float.floatToRawIntBits(movement.h) +
                        " i=" + movement.i + ":" + Float.floatToRawIntBits(movement.i));
    }
    ...
    public TriggerDebugAction a(UnitCommand unitCommand, UnitType unitType, int i, float f, float f2) {
        ...
        boolean probeBuildPlacement = shouldProbeBuildPlacement(gameEngine);
        BaseUnit baseUnitG = BaseBuilding.g(unitType);
        if (probeBuildPlacement) {
            logBuildPlacementProbe(
                    gameEngine,
                    "created",
                    unitCommand,
                    unitType,
        ...
        if (baseUnitG instanceof OrderableUnit) {
            ...
            boolean hasBlockingUnitNearby = orderableUnit.hasBlockingUnitNearby(orderableUnit2, (PlayerTeam) null);
            boolean canPlaceAtCurrentPositionWithIgnoreUnits = false;
            if (hasBlockingUnitNearby) {
                z = true;
            }
            if (!z) {
                canPlaceAtCurrentPositionWithIgnoreUnits = orderableUnit.canPlaceAtCurrentPositionWithIgnoreUnits(true, (PlayerTeam) null);
                if (!canPlaceAtCurrentPositionWithIgnoreUnits) {
                    z = true;
                }
            }
            if (probeBuildPlacement) {
                logBuildPlacementProbe(
                        ...
                        f2,
                        baseUnitG,
                        hasBlockingUnitNearby,
                        canPlaceAtCurrentPositionWithIgnoreUnits,
                        z,
                        orderableUnit2);
            }
            if (z) {
        ...
        boolean canAffordDisplayText = displayText.c(this);
        if (probeBuildPlacement) {
            logBuildPlacementProbe(
                    gameEngine,
                    "price",
                    ...
                    false,
                    false,
                    !canAffordDisplayText,
                    null);
        }
        if (!canAffordDisplayText) {
            baseUnitG.remove();
    ...
    private static boolean shouldProbeBuildPlacement(GameEngine gameEngine) {
        if (!"1".equals(System.getenv("RWX_PROBE_BUILD_PLACEMENT"))) {
            return false;
        }
        String startValue = System.getenv("RWX_PROBE_BUILD_PLACEMENT_START_TICK");
        String endValue = System.getenv("RWX_PROBE_BUILD_PLACEMENT_END_TICK");
        if (startValue == null || endValue == null) {
            return true;
        }
        try {
            int tick = gameEngine.currentTick;
            return tick >= Integer.parseInt(startValue) && tick <= Integer.parseInt(endValue);
        } catch (NumberFormatException ignored) {
            return false;
        }
    }

    private void logBuildPlacementProbe(
            ...
            float targetY,
            BaseUnit buildUnit,
            boolean hasBlockingUnitNearby,
            boolean canPlaceAtCurrentPositionWithIgnoreUnits,
            boolean rejects,
            OrderableUnit ignoredUnit) {
        System.out.println(
                "BUILD_PLACEMENT tick=" + gameEngine.currentTick +
                        " phase=" + phase +
                        " builder=" + this.objectId +
                        ":" + r().getUnitTypeDescriptionShort() +
                        ":team=" + (this.team == null ? "null" : this.team.teamId) +
                        " buildUnit=" + buildUnit.objectId +
                        ":" + buildUnit.getClass().getSimpleName() +
                        ":" + unitType.getUnitTypeDescriptionShort() +
                        ":team=" + (buildUnit.team == null ? "null" : buildUnit.team.teamId) +
                        " queueSize=" + queueSize +
                        " force=" + unitCommand.isForceMove +
                        " requested=" + targetX + "," + targetY +
                        " placed=" + buildUnit.posX + "," + buildUnit.posY +
                        " nextUnitId=" + gameEngine.networkEngine.nextUnitId +
                        " ignored=" + (ignoredUnit == null ? "null" : ignoredUnit.objectId) +
                        " hasBlocking=" + hasBlockingUnitNearby +
                        " canPlaceIgnore=" + canPlaceAtCurrentPositionWithIgnoreUnits +
                        " rejects=" + rejects);
    }
