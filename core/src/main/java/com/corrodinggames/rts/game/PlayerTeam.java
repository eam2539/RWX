...
public abstract class PlayerTeam extends Serializable implements Comparable<PlayerTeam> {
    ...
    public static Texture[] b(Texture texture, ColorMode colorMode) {
        ...
        int[] teamColors = new int[10];
        int[] teamIndexes = new int[10];
        for (int i = 0; i < teamColors.length; i++) {
            teamColors[i] = i(i);
            teamIndexes[i] = i;
        }
        texture.j();
        int[] sourcePixels = texture.getArgbPixelsCopy();
        for (int i = 1; i < textureArr.length; i++) {
            textureArr[i] = texture.a(texture.m(), texture.l(), false);
            textureArr[i].a("color(" + i + "):" + texture.a());
        }
        if (sourcePixels != null) {
            for (int i = 1; i < textureArr.length; i++) {
                int[] coloredPixels = new int[sourcePixels.length];
                int teamColor = teamColors[i];
                for (int pixelIndex = 0; pixelIndex < sourcePixels.length; pixelIndex++) {
                    ...
                    if (colorMode == ColorMode.hueAdd) {
                        coloredPixels[pixelIndex] = TeamColoring.hueAdd(sourceColor, teamColor);
                    } else if (colorMode == ColorMode.hueShift) {
                        coloredPixels[pixelIndex] = TeamColoring.hueShift(sourceColor, teamColor);
                    } else {
                        coloredPixels[pixelIndex] = TeamColoring.pureGreen(sourceColor, teamColor);
                    }
                }
                textureArr[i].setCommittedArgbPixels(coloredPixels);
            }
        } else {
            if (colorMode == ColorMode.hueAdd) {
                b(texture, textureArr, teamColors);
            } else if (colorMode == ColorMode.hueShift) {
                a(texture, textureArr, teamColors, teamIndexes);
            } else {
                a(texture, textureArr, teamColors);
            }
        }
        for (Texture teamTexture : textureArr) {
            if (teamTexture != null) {
                teamTexture.p();
                teamTexture.s();
            }
    ...
    public static void a(Texture texture, Texture[] textureArr, int[] teamColors) {
        int width = texture.m();
        int height = texture.l();
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                int color = texture.a(x, y);
                for (int i = 0; i < textureArr.length; i++) {
                    if (textureArr[i] != null) {
                        textureArr[i].a(x, y, TeamColoring.pureGreen(color, teamColors[i]));
                    }
                }
            }
        }
    ...
    public static void a(Texture texture, Texture[] textureArr, int[] teamColors, int[] teamIndexes) {
        int width = texture.m();
        int height = texture.l();
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                for (int i = 0; i < textureArr.length; i++) {
                    if (textureArr[i] != null) {
                        textureArr[i].a(x, y, TeamColoring.hueShift(color, teamColors[i]));
                    }
                }
            }
        }
    ...
    public static void b(Texture texture, Texture[] textureArr, int[] teamColors) {
        int width = texture.m();
        int height = texture.l();
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                for (int i = 0; i < textureArr.length; i++) {
                    if (textureArr[i] != null) {
                        textureArr[i].a(x, y, TeamColoring.hueAdd(color, teamColors[i]));
                    }
                }
            }
        }
