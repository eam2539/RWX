...
public class MissionEngine extends Serializable {
    ...
    MapPortalMode mapPortalMode;
    boolean areaControlEditorModified;
    ...
    public void a(GameOutputStream gameOutputStream) throws IOException {
        ...
        if (missionStateVersion >= 7) {
            ...
            if (hasAreaControlMode) {
                if (missionStateVersion >= 8) {
                    this.areaControlMode.writeSnapshot(gameOutputStream);
                } else {
                    this.areaControlMode.writeState(gameOutputStream);
                }
            }
        }
        if (missionStateVersion >= 8) {
            gameOutputStream.writeBoolean(hasMapPortalMode);
            if (hasMapPortalMode) {
                this.mapPortalMode.writeSnapshot(gameOutputStream);
            }
        }
    }
    ...
    public void a(GameInputStream gameInputStream) throws IOException {
        ...
        if (i >= 7) {
            ...
            if (hasAreaControlMode) {
                if (i >= 8) {
                    this.areaControlMode = AreaControlMode.readSnapshot(gameInputStream, GameEngine.getInstance().tileMap.objectsLayer);
                } else if (this.areaControlMode != null) {
                    this.areaControlMode.readState(gameInputStream);
                } else {
                    AreaControlMode.skipState(gameInputStream);
                }
            }
        }
        if (i >= 8) {
            boolean hasMapPortalMode = gameInputStream.readBoolean();
            if (hasMapPortalMode) {
                this.mapPortalMode = MapPortalMode.readSnapshot(gameInputStream, GameEngine.getInstance().tileMap.objectsLayer);
            } else {
    ...
    public void a(final boolean boolean1) throws MapLoadException {
        ...
        this.E.a(KoolTypeface.a(KoolTypeface.c, 1));
        instance.updatePaintTextSize(this.E, 24.0f);
        (this.G = new KoolPaint()).a(255, 255, 255, 255);
        this.G.a(true);
        this.G.a(KoolPaint.Align.CENTER);
        instance.updatePaintTextSize(this.G, 18.0f);
        (this.H = new KoolPaint()).a(255, 255, 255, 255);
        this.H.a(true);
        this.H.a(KoolPaint.Align.CENTER);
    ...
    public int addAreaControlEditorZoneBounds(float left, float top, float right, float bottom, boolean circle) throws MapLoadException {
        ...
        if (circle) {
            ...
            return addAreaControlEditorZone(centerX, centerY, true, diameter, diameter);
        }
        if (width <= 0.0f) {
            width = 420.0f;
        }
        ...
        float centerX = (left + right) * 0.5f;
        float centerY = (top + bottom) * 0.5f;
        return addAreaControlEditorZone(centerX, centerY, false, width, height);
    }

    public String removeAreaControlEditorZone(float x, float y) throws MapLoadException {
        ...
        MapObject removed = this.areaControlMode.removeEditorZoneAt(x, y);
        if (removed == null) {
            return null;
        }
        if (gameEngine.tileMap.objectsLayer != null && gameEngine.tileMap.objectsLayer.mapObjects != null) {
            gameEngine.tileMap.objectsLayer.mapObjects.remove(removed);
        }
        this.areaControlEditorModified = true;
        if (this.areaControlMode.getZoneCount() == 0) {
            this.areaControlMode = null;
        }
        String id = removed.getDescription("id");
    ...
    public AreaControlMode.EditorZoneProperties getAreaControlEditorZone(float x, float y) throws MapLoadException {
        GameEngine gameEngine = GameEngine.getInstance();
        if (gameEngine == null || gameEngine.tileMap == null) {
            throw new MapLoadException("No map loaded");
        }
    ...
    public String updateAreaControlEditorZone(float x, float y, AreaControlMode.EditorZoneProperties properties) throws MapLoadException {
        ...
        String id = this.areaControlMode.updateEditorZoneAt(x, y, properties);
        if (id != null) {
            this.areaControlEditorModified = true;
        }
        return id;
    ...
    public int addMapPortalEditorPortalBounds(float left, float top, float right, float bottom, boolean circle, String targetMapId, String targetPortalId) throws MapLoadException {
        ...
        if (circle) {
            ...
            float centerX = (left + right) * 0.5f;
            float centerY = (top + bottom) * 0.5f;
            return addMapPortalEditorPortal(centerX, centerY, true, diameter, diameter, targetMapId, targetPortalId);
        }
        if (width <= 0.0f) {
            width = 360.0f;
        }
        ...
        float centerX = (left + right) * 0.5f;
        float centerY = (top + bottom) * 0.5f;
        return addMapPortalEditorPortal(centerX, centerY, false, width, height, targetMapId, targetPortalId);
    }

    public String removeMapPortalEditorPortal(float x, float y) throws MapLoadException {
        GameEngine gameEngine = GameEngine.getInstance();
        if (gameEngine == null || gameEngine.tileMap == null) {
            throw new MapLoadException("No map loaded");
        }
        if (this.mapPortalMode == null || this.mapPortalMode.getPortalCount() == 0) {
            return null;
        }
        MapObject removed = this.mapPortalMode.removeEditorPortalAt(x, y);
        if (removed == null) {
            return null;
        }
        if (gameEngine.tileMap.objectsLayer != null && gameEngine.tileMap.objectsLayer.mapObjects != null) {
        ...
        String id = removed.getDescription("id");
        return id == null || id.trim().length() == 0 ? removed.name : id;
    }

    public MapPortalMode.EditorPortalProperties getMapPortalEditorPortal(float x, float y) throws MapLoadException {
        GameEngine gameEngine = GameEngine.getInstance();
        if (gameEngine == null || gameEngine.tileMap == null) {
            throw new MapLoadException("No map loaded");
        }
        ...
        return this.mapPortalMode.getEditorPortalPropertiesAt(x, y);
    }

    public String updateMapPortalEditorPortal(float x, float y, MapPortalMode.EditorPortalProperties properties) throws MapLoadException {
        GameEngine gameEngine = GameEngine.getInstance();
        if (gameEngine == null || gameEngine.tileMap == null) {
            throw new MapLoadException("No map loaded");
        }
    ...
    public void exportEditorMap(String targetPath) throws MapLoadException, IOException {
        ...
        AreaControlMode areaMode = this.areaControlMode;
        MapPortalMode portalMode = this.mapPortalMode;
        ...
        if (hasAreaZones) {
            areaMode.writeEditorZonesToPath(targetPath);
        } else if (this.areaControlEditorModified) {
            AreaControlMode.clearEditorZonesFromPath(targetPath);
        }
        if (hasMapPortals) {
            portalMode.writeEditorPortalsToPath(targetPath);
        }
    }

    private MapObjectLayer ensureAreaControlEditorObjectLayer(GameEngine gameEngine) throws MapLoadException {
    ...
    private MapObject createAreaControlEditorObject(GameEngine gameEngine, MapObjectLayer layer, int zoneIndex, float centerX, float centerY, boolean circle, float width, float height) throws MapLoadException {
        try {
            Document document = DocumentBuilderFactory.newInstance().newDocumentBuilder().newDocument();
            Element object = document.createElement("object");
            String id = editorZoneId(zoneIndex);
            object.setAttribute("id", String.valueOf(10000 + zoneIndex));
            object.setAttribute("name", id);
            object.setAttribute("type", AreaControlMode.CONTROL_ZONE_TYPE);
            ...
            addEditorProperty(document, properties, "captureTime", "8");
            addEditorProperty(document, properties, "neutralizeTime", "4");
            addEditorProperty(document, properties, "scoreRate", "1");
            addEditorProperty(document, properties, "scoreInterval", "3");
            addEditorProperty(document, properties, "groundOnly", "true");
    ...
    private static void addEditorProperty(Document document, Element properties, String name, String value) {
    ...
    private static String editorZoneId(int index) {
        if (index >= 0 && index < 26) {
    ...
    public void b(final float float1) {
        ...
        if (this.areaControlMode != null) {
            this.areaControlMode.draw(this.G, this.H);
        }
        if (this.mapPortalMode != null) {
