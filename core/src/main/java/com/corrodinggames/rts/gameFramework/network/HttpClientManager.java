...
public class HttpClientManager {
    ...
    public HttpClient getNewHttpClient(int i, boolean z) {
        ...
        if (!z && GameEngine.isAndroidPlatform()) {
            PlatformBridge platformBridge = KoinJavaComponent.get(PlatformBridge.class);
            defaultHttpClient = platformBridge.createMasterServerHttpClient();
        } else {
    ...
    public void closeIfAndroidClient(HttpClient httpClient) {
        if (GameEngine.isAndroidPlatform()) {
            PlatformBridge platformBridge = KoinJavaComponent.get(PlatformBridge.class);
            platformBridge.closeMasterServerHttpClient(httpClient);
        }
    }
}
